...
void SPI_Config(){
	...
	SPI_InitStruct.SPI_Mode = SPI_Mode_Master; //chon lam master/slave
...
void SPI_TransmitReceive(uint8_t *data_Transmit, uint8_t *data_Receive, uint8_t length){
	GPIO_ResetBits(PORT1, SS);
	delayMs(1);
	...
	for(; i<length; i++){
		...
		if(i==length-1) GPIO_SetBits(PORT1, SS);
